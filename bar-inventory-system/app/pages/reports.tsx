  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-100">
                {[...INVENTORY].sort((a,b) => b.sold - a.sold).map(item => {
                  ...
                  return (
                    <tr key={item.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3">{item.sold.toLocaleString()}</td>
                      <td className="px-4 py-3 font-medium text-brand">KES {(item.price * item.sold).toLocaleString()}</td>
        ...
        {/* M-Pesa price group detail */}
        <div className="bg-white border border-gray-200 rounded-xl p-5">
          <h2 className="text-sm font-medium mb-4">M-Pesa Payment Groups Breakdown</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            {PRICE_GROUPS.map(g => (
              <div key={g.label} className="border border-gray-200 rounded-xl p-4">
                ...
                <div className="text-xs text-gray-500 mt-0.5">{g.label} — KES {g.total.toLocaleString()}</div>
                <div className="mt-2 h-1 bg-gray-100 rounded-full overflow-hidden">
                  <div className="h-full rounded-full" style={{ background: g.color, width: `${Math.round(g.count/totalTxns*100)}%` }} />
                </div>
              </div>
            ))}
          </div>
